        ...
        >
            <a-form :labelCol="{ span: 6, offset: 0 }" :wrapper-col="{span:18}">
                <a-row :gutter="16" justify="start">
                        ...
                        <a-form-item  label="订单标题：" v-bind="searchValidateInfos.title">
                            <a-input placeholder="请输入" v-model:value="searchModelRef.title" />
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
                        <a-form-item  label="支付渠道：aliPay 支付宝， wxPay微信..等等：" v-bind="searchValidateInfos.payType">
                            <a-input placeholder="请输入" v-model:value="searchModelRef.payType" />
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
                        <a-form-item  label="下单方式：app wap page 等：" v-bind="searchValidateInfos.payWay">
                            <a-input placeholder="请输入" v-model:value="searchModelRef.payWay" />
                        </a-form-item>
                    </a-col>
                </a-row>
                ...
                <template #action="{ record }">
                    <a-row type="flex" justify="center">
                        <a-col flex="1" offset=2>
                            <a-button type="primary" size="small" @click.stop="() => detailUpdateData(record.id)" :loading="detailUpdateLoading.includes(record.id)">编辑</a-button>
                        </a-col>
                        <a-col flex="1" offset=2>
                            <a-button type="danger" size="small" @click.stop="() => deleteTableData(record.id)" :loading="deleteLoading.includes(record.id)">删除</a-button>
                        </a-col>
                    </a-row>
                </template>
            </a-table>
...
<script lang="ts">
    ...
    import { PaginationConfig, PayOrderDataType, TableListQueryParams } from './data.d';
    import moment from 'moment';

    interface ListSearchTablePageSetupData {
        columns: any;
        list: PayOrderDataType[];
        ...
        createSubmit: (values: Omit<PayOrderDataType, 'id'>, resetFields: (newValues?: Props | undefined) => void) => Promise<void>;
        detailUpdateLoading: number[];
        detailUpdateData: (id: number) => Promise<void>;
        updateData: Partial<PayOrderDataType>;
        updateFormVisible: boolean;
        updataFormCancel:  () => void;
        updateSubmitLoading: boolean;
        updateSubmit:  (values: PayOrderDataType, resetFields: (newValues?: Props | undefined) => void) => Promise<void>;
        ...
        rowClick: (record: PayOrderDataType, index: number) => {};
        rowSelection: {};
    ...
    export default defineComponent({
        name: 'PayOrderListSearchTablePage',
        components: {
            CreateForm,
            UpdateForm,
            DownOutlined,
        ...
        setup(): ListSearchTablePageSetupData {
            ...
            const rowClick = (record: PayOrderDataType, index: number) => {
                return {
                    // 点击行
                    onClick:() => {
                        console.log(record);
            ...
            const rowSelection = {
                onChange: (selectedRowKeys: (string | number)[], selectedRows: PayOrderDataType[]) => {
                    // console.log('onChange');
                    // console.log(`selectedRowKeys: ${selectedRowKeys}`, 'selectedRows: ', selectedRows);

                },
                onSelect: (record: PayOrderDataType, selected: boolean, selectedRows: PayOrderDataType[]) => {
                    // console.log('---------onSelect-----------');
                    // console.log(record, selected, selectedRows);
                    selectIds.value = selectedRows.map((item)=> {return item.id});
                },
                onSelectAll: (selected: boolean, selectedRows: PayOrderDataType[], changeRows: PayOrderDataType[]) => {
            ...
            const store = useStore<{ PayOrderListSearchTable: ListStateType}>();

            // 列表数据
            const list = computed<PayOrderDataType[]>(() => store.state.PayOrderListSearchTable.tableData.list);

            // 列表分页
            const pagination = computed<PaginationConfig>(() => store.state.PayOrderListSearchTable.tableData.pagination);
            ...
            const columns =[
                ...
                {
                    title: '支付渠道：aliPay 支付宝， wxPay微信..等等',
                    dataIndex: 'payType',
                    width: 120,
                    
                },
                {
                    title: '下单方式：app wap page 等',
                    dataIndex: 'payWay',
                    width: 120,
            ...
            const getList = async (current: number, pageSize: number): Promise<void> => {
                loading.value = true;
                try {
                    const filedsValue = await validate<Omit<PayOrderDataType, 'id'>>();

                    await store.dispatch('PayOrderListSearchTable/queryTableData', {
                        ...filedsValue,
                        ...{
                            page: current,
                            size: pageSize,
            ...
            // 新增弹框 - 提交
            const createSubmit = async (values: Omit<PayOrderDataType, 'id'>, resetFields: (newValues?: Props | undefined) => void) => {
                createSubmitLoading.value = true;
                const res: boolean = await store.dispatch('PayOrderListSearchTable/createTableData',values);
                if(res === true) {
                    resetFields();
            ...
            const updataFormCancel = () => {
                ...
                store.commit('PayOrderListSearchTable/setUpdateData',{});
            }
            // 编辑弹框 - 提交 loading
            const updateSubmitLoading = ref<boolean>(false);
            // 编辑弹框 - 提交
            const updateSubmit = async (values: PayOrderDataType, resetFields: (newValues?: Props | undefined) => void) => {
                updateSubmitLoading.value = true;
                console.log("values:",values);
                console.log("newValues:",values);
                const res: boolean = await store.dispatch('PayOrderListSearchTable/updateTableData',values);
                if(res === true) {
                    updataFormCancel();
            ...
            // 编辑弹框 data
            const updateData = computed<Partial<PayOrderDataType>>(() => store.state.PayOrderListSearchTable.updateData);
            const detailUpdateLoading = ref<(string | number)[]>([]);
            const detailUpdateData = async (id: number) => {
                detailUpdateLoading.value = [id];
                const res: boolean = await store.dispatch('PayOrderListSearchTable/queryUpdateData', id);
                if(res===true) {
            ...
            const deleteTableData = (id: number) => {
                Modal.confirm({
                    ...
                    onOk: async () => {
                        ...
                        const res: boolean = await store.dispatch('PayOrderListSearchTable/deleteTableData',id);
                        if (res === true) {
            ...
            const batchDeleteTableData = async ()=>{
                ...
                Modal.confirm({
                    ...
                    onOk: async () => {
                        ...
                        const res: boolean = await store.dispatch('PayOrderListSearchTable/batchDeleteTableData',selectIds.value);
                        if (res === true) {
                            message.success('删除成功！');
                            getList(pagination.value.current, pagination.value.pageSize);
                        }
            ...
            return {
                columns,
                list: list as unknown as PayOrderDataType[],
                ...
                detailUpdateData,
                updateData: updateData as Partial<PayOrderDataType>,
