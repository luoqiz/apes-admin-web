                ...
                <template #action="{ record }">
                    <a-row type="flex" justify="center">
                        <a-col flex="1" offset=2>
                            <a-button type="primary" size="small" @click.stop="() => detailUpdateData(record.jobId)" :loading="detailUpdateLoading.includes(record.jobId)">编辑</a-button>
                        </a-col>
                        <a-col flex="1" offset=2>
                            <a-button type="danger" size="small" @click.stop="() => deleteTableData(record.jobId)" :loading="deleteLoading.includes(record.jobId)">删除</a-button>
                        </a-col>
                    </a-row>
                </template>
        ...
        setup(): ListSearchTablePageSetupData {
            ...
            const rowSelection = {
                ...
                onSelect: (record: SysJobDataType, selected: boolean, selectedRows: SysJobDataType[]) => {
                    ...
                    selectIds.value = selectedRows.map((item)=> {return item.jobId});
                },
