                ...
                <template #action="{ record }">
                    <a-row type="flex" justify="center">
                        <a-col flex="1" offset=2>
                            <a-button type="primary" size="small" @click.stop="() => detailUpdateData(record.menuId)" :loading="detailUpdateLoading.includes(record.menuId)">编辑</a-button>
                        </a-col>
                        <a-col flex="1" offset=2>
                            <a-button type="danger" size="small" @click.stop="() => deleteTableData(record.menuId)" :loading="deleteLoading.includes(record.menuId)">删除</a-button>
                        </a-col>
                    </a-row>
                </template>
            </a-table>
        ...
        setup(): ListSearchTablePageSetupData {
            ...
            const rowSelection = {
                ...
                onSelect: (record: SysMenuDataType, selected: boolean, selectedRows: SysMenuDataType[]) => {
                    ...
                    selectIds.value = selectedRows.map((item)=> {return item.menuId});
                },
            ...
            const getList = async (current: number, pageSize: number): Promise<void> => {
                loading.value = true;
                try {
                    const filedsValue = await validate<Omit<SysMenuDataType, 'menuId'>>();
