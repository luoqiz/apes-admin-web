                ...
                <template #action="{ record }">
                    <a-row type="flex" justify="center">
                        <a-col flex="1" offset=2>
                            <a-button type="primary" size="small" @click.stop="() => detailUpdateData(record.detailsId)" :loading="detailUpdateLoading.includes(record.detailsId)">编辑</a-button>
                        </a-col>
                        <a-col flex="1" offset=2>
                            <a-button type="danger" size="small" @click.stop="() => deleteTableData(record.detailsId)" :loading="deleteLoading.includes(record.detailsId)">删除</a-button>
                        </a-col>
                    </a-row>
                </template>
            </a-table>
...
<script lang="ts">
    ...
    import { PaginationConfig, MerchantDetailsDataType, TableListQueryParams } from './data.d';
        ...
        setup(): ListSearchTablePageSetupData {
            ...
            const rowSelection = {
                ...
                onSelect: (record: MerchantDetailsDataType, selected: boolean, selectedRows: MerchantDetailsDataType[]) => {
                    ...
                    selectIds.value = selectedRows.map((item)=> {return item.detailsId});
                },
