            ...
            <a-form :labelCol="{ span: 6, offset: 0 }" :wrapper-col="{span:18}">
                <a-row :gutter="16" justify="start">
                        ...
                        <a-form-item  label="上级部门：" v-bind="searchValidateInfos.pid">
                            <a-input placeholder="请输入" v-model:value="searchModelRef.pid" />
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
                ...
                <template #action="{ record }">
                    <a-row type="flex" justify="center">
                        <a-col flex="1" offset=2>
                            <a-button type="primary" size="small" @click.stop="() => detailUpdateData(record.deptId)" :loading="detailUpdateLoading.includes(record.deptId)">编辑</a-button>
                        </a-col>
                        <a-col flex="1" offset=2>
                            <a-button type="danger" size="small" @click.stop="() => deleteTableData(record.deptId)" :loading="deleteLoading.includes(record.deptId)">删除</a-button>
                        </a-col>
                    </a-row>
                </template>
            </a-table>
        ...
        setup(): ListSearchTablePageSetupData {
            ...
            const rowSelection = {
                ...
                onSelect: (record: SysDeptDataType, selected: boolean, selectedRows: SysDeptDataType[]) => {
                    ...
                    selectIds.value = selectedRows.map((item)=> {return item.deptId});
                },
            ...
            const getList = async (current: number, pageSize: number): Promise<void> => {
                loading.value = true;
                try {
                    const filedsValue = await validate<Omit<SysDeptDataType, 'deptId'>>();
