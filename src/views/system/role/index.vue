                ...
                <template #action="{ record }">
                    <a-row type="flex" justify="center">
                        <a-col flex="1" offset=2>
                            <a-button type="primary" size="small" @click.stop="() => detailUpdateData(record.roleId)" :loading="detailUpdateLoading.includes(record.roleId)">编辑</a-button>
                        </a-col>
                        <a-col flex="1" offset=2>
                            <a-button type="danger" size="small" @click.stop="() => deleteTableData(record.roleId)" :loading="deleteLoading.includes(record.roleId)">删除</a-button>
                        </a-col>
                    </a-row>
                </template>
            </a-table>
...
<script lang="ts">
    ...
    import { PaginationConfig, SysRoleDataType, TableListQueryParams } from './data.d';
        ...
        setup(): ListSearchTablePageSetupData {
            ...
            const rowSelection = {
                ...
                onSelect: (record: SysRoleDataType, selected: boolean, selectedRows: SysRoleDataType[]) => {
                    ...
                    selectIds.value = selectedRows.map((item)=> {return item.roleId});
                },
            ...
            const getList = async (current: number, pageSize: number): Promise<void> => {
                loading.value = true;
                try {
                    const filedsValue = await validate<Omit<SysRoleDataType, 'roleId'>>();
