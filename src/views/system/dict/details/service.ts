...
export async function updateData(detailId: number, params: Omit<SysDictDetailDataType, 'detailId'>): Promise<any> {
    return request({
        url: `SysDictDetail`,
        method: 'PUT',
        data: params,
...
export async function removeData(detailId: number): Promise<any> {
    return request({
        url: `SysDictDetail/${detailId}`,
        method: 'delete',
    });
}

export async function detailData(detailId: number): Promise<any> {
    return request({url: `SysDictDetail/${detailId}`});
}