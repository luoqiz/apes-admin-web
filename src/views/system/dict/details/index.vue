<template>
        ...
        <a-card :bordered="false">
            <template #title>
                <a-button type="primary" @click="() => setCreateFormVisible(true)" :disabled="dictIdForm === '' || !dictIdForm || dictIdForm ==='0'">新增</a-button>
            </template>
            <a-table
                    row-key="detailId"
                    :columns="columns"
                ...
                <template #action="{ record }">
                    <a-row type="flex" justify="center">
                        <a-col flex="1" offset=2>
                            <a-button type="primary" size="small" @click.stop="() => detailUpdateData(record.detailId)" :loading="detailUpdateLoading.includes(record.detailId)">编辑</a-button>
                        </a-col>
                        <a-col flex="1" offset=2>
                            <a-button type="danger" size="small" @click.stop="() => deleteTableData(record.detailId)" :loading="deleteLoading.includes(record.detailId)">删除</a-button>
                        </a-col>
                    </a-row>
                </template>
